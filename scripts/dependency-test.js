...
function dependencyDiff(existingPackageLock : PackageLock, candidatePackageLock : PackageLock) : DependenciesDiff {
    const existingPackageLockDependencies = flattenDependencies(existingPackageLock);
    const candidatePackageLockDependencies = flattenDependencies(candidatePackageLock);

    const result : DependenciesDiff = {};

    for (const [ name, candidateVersion ] of Object.entries(candidatePackageLockDependencies)) {
...
async function runTests(moduleName : string, packageLock : PackageLock) : Promise<void> {
...
async function run() {
    ...
    if (!JSON.stringify(candidatePackageLock) === JSON.stringify(existingPackageLock)) {
    ...
    // $FlowFixMe
    for (const [ name, { existingVersion, candidateVersion } ] of Object.entries(diff)) {
        console.info(`- ${ name }: ${ existingVersion } -> ${ candidateVersion }`);
    }
    console.info('');

    const dependentModules = findAllDependentModules(existingPackageLock, Object.keys(diff));

    console.info(`Found the following dependent modules for ${ Object.keys(diff).join(', ') }:`);
    // $FlowFixMe
    for (const name of Object.keys(dependentModules)) {
        console.info(`- ${ name }: ${ Object.keys(dependentModules[name]).join(', ') }`);
    }
    console.info('');
    ...
    await Promise.all(Object.keys(dependentModules).map(async moduleName => {
        try {
