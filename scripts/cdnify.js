  ...
  const options = commandLineArgs(
    [
      ...
      { name: "cdn", type: String, defaultValue: process.env.CDN || "" },
      {
        name: "namespace",
        type: String,
        defaultValue: process.env.NAMESPACE || "",
      },
      {
        name: "infofile",
        type: String,
        defaultValue: process.env.INFO_FILE || "info.json",
      },
      {
        name: "tarballfolder",
        type: String,
        defaultValue: process.env.TARBALL_FOLDER || "tarballs",
      },
      {
        name: "cdnpath",
        type: String,
        defaultValue: process.env.CDN_PATH || join(process.cwd(), "cdn"),
      },
      {
        name: "recursive",
        type: Boolean,
        defaultValue: booleanEnv(process.env.RECURSIVE),
      },
      {
        name: "package",
        type: String,
        defaultValue:
          process.env.PACKAGE || join(process.cwd(), "package.json"),
      },
      {
        name: "nodeops",
        type: String,
        defaultValue: process.env.NODE_OPS || join(process.cwd(), ".nodeops"),
      },
      {
        name: "cdnapi",
        type: String,
        defaultValue: process.env.CDNAPI || "https://cdnx-api.qa.paypal.com",
      },
      {
        name: "requester",
        type: String,
        defaultValue: process.env.REQUESTER || "svc-xo",
      },
      {
        name: "password",
        type: String,
        defaultValue: process.env.SVC_PASSWORD,
      },
      {
        name: "approver",
        type: String,
        defaultValue: process.env.APPROVER || userInfo().username,
      },
      {
        name: "disttag",
        type: String,
        defaultValue: process.env.DIST_TAG || "latest",
      },
      {
        name: "deployonly",
        type: Boolean,
        defaultValue: booleanEnv(process.env.DEPOY_ONLY),
      },
      {
        name: "commitonly",
        type: Boolean,
        defaultValue: booleanEnv(process.env.COMMIT_ONLY),
      },
      {
        name: "versionsToKeep",
        type: Number,
        defaultValue: process.env.VERSIONS_TO_KEEP || 0,
      },
...
|}) => {
  ...
  if (parentName) {
    return unique(
      versionsToKeep.map(
        (parentActiveVersion) =>
          versions[parentActiveVersion].dependencies[name]
      )
    );
  } else {
...
const cdnifyGenerateModule = async (
  {
    cdnNamespace,
    name,
    version,
  ...
  for (const existingVersion of Object.keys(pkgInfo.versions)) {
    if (!versionsToKeep.includes(existingVersion)) {
      ...
      const existingVersionTarballPath = join(
        cdnModuleTarballDir,
        `${existingVersion}${extname(versionConfig.dist.tarball)}`
      );
  ...
  for (const [moduleVersion, moduleVersionInfo] of Object.entries(
    pkgInfo.versions
  )) {
    const moduleVersionTarballFileName = `${moduleVersion}${extname(tarball)}`;
    const moduleVersionTarballFile = join(
      cdnModuleTarballDir,
      moduleVersionTarballFileName
    );

    if (await exists(moduleVersionTarballFile)) {
      const relativeTarballDir = join(
        sanitizedName,
        options.tarballfolder,
        moduleVersionTarballFileName
      );
...
const cdnifyDeploy = async (options) => {
  ...
  const approveRes = await fetch(
    `${options.cdnapi}/assets/approve/${id}?requestor=${options.requester}&approver=${options.approver}`
  );
  ...
  if (!approveRes.ok) {
    ...
    if (
      !(await getYesNo(
        `Approval failed with status ${approveRes.status}. Please try to approve manually.\n\nhttps://cdnx-ui.qa.paypal.com/approve/${id}\n\nContinue with release?`
      ))
    ) {
