...
if (!DIST_TAG || !LOCAL_VERSION) {
  throw new Error(
    "Invalid arguments. Expected <LOCAL_VERSION> and <DIST_TAG>."
  );
}
...
let { stdout: npm_public_registry_version } =
  await $`npm view ${PACKAGE_NAME} dist-tags.${DIST_TAG}`;
...
while (LOCAL_VERSION !== npm_public_registry_version) {
  ...
  console.log(
    `Version mismatch between local version ${LOCAL_VERSION} and npm version ${npm_public_registry_version}. Trying again in ${interval} seconds...`
  );
  await $`sleep ${interval}`;
  npm_public_registry_version =
    await $`npm view ${PACKAGE_NAME} dist-tags.${DIST_TAG}`;
