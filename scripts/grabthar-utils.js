...
export const dns = async (host : string, family? : number = 6) : Promise<string> => {
...
export async function exec<T>(cmd : string, envVars? : {| [string] : string |}) : Promise<T> {
    ...
    const result = await new Promise((resolve, reject) => {
        shell.exec(cmd, { silent: !options.verbose, async: true }, (code, stdout, stderr) => {
            if (code !== 0) {
                let message;
...
export const info = async (name : string, expectedDistTag? : string) : Promise<PackageInfo> => {
    ...
    if (expectedDistTag && (!json['dist-tags'] || !json['dist-tags'][expectedDistTag])) {
...
export const getDistVersions = async (name : string) : Promise<$ReadOnlyArray<string>> => {
...
export function findDependentModules(packageLock : PackageLock, dependendencyName : string) : {| [string] : {| [string] : string |} |} {
    const result = {};
...
export function findAllDependentModules(packageLock : PackageLock, dependencyNames : $ReadOnlyArray<string>) : {| [string] : {| [string] : string |} |} {
    const result = {};
    ...
    for (const dependencyName of dependencyNames) {
        ...
        for (const dependentModuleName of Object.keys(dependentModules)) {
            ...
            result[dependentModuleName] = { ...result[dependentModuleName], ...dependentModules[dependentModuleName] };
        }
    }
...
export async function generateTempDirectory(name : string) : Promise<string> {
    const dir = join(tmpdir(), `${ name }-${ Math.random().toString().split('.')[1] }`);
    await ensureDir(dir);
    return dir;
}
...
export async function writePackageForDir(dir : string, pkg : Package) : Promise<void> {
    const pkgPath = join(dir, PACKAGE_JSON_NAME);
    await writeFile(pkgPath, JSON.stringify(pkg, null, 2));
}

export async function updatePackageWithLockedDependencies(dir : string, packageLock : PackageLock) : Promise<void> {
